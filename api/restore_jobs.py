...
class RestoreJob:
    ...
    result: schemas.RestoreResponse | None = None
    error: str | None = None
...
class RestoreJobManager:
    ...
    def submit(
        ...
        request_id: str,
        password: str,
        selection_expr: str | None,
        structured: list[dict[str, list[int] | None]] | None,
        *,
        ...
        with self._lock:
            if any(job.status in {"pending", "running"} for job in self._jobs.values()):
                raise RestoreJobConflictError(
                    "A restore is already running. Wait for it to complete."
                )
    ...
    def _run_job(
        ...
        job.status = "running"
        logger.info(
            "Restore job %s started (request_id=%s).", job.job_id, job.request_id
        )
        try:
